...
write_webdav_config() {
  ...
  WEBDAV_USERNAME_INPUT="${WEBDAV_USERNAME}" \
  WEBDAV_PASSWORD_INPUT="${WEBDAV_PASSWORD}" \
  python3 - "${ROOT_DIR}/webdav/config.yml" <<'PY'
...
username = os.environ["WEBDAV_USERNAME_INPUT"]
password = os.environ["WEBDAV_PASSWORD_INPUT"]
auth_rule = f"{username}:{password}@/"

content = f"""# Generated by scripts/bootstrap-remote.sh on the deployment host.
# dufs serves the media tree in read-only mode and supports both:
#   - browser navigation at /webdav/
#   - WebDAV clients against the same /webdav/ path
serve-path: /media
bind: 0.0.0.0
port: 6065
path-prefix: /webdav
