...
write_webdav_config() {
  mkdir -p "${ROOT_DIR}/webdav"

  # Auth is handled by Caddy's basic_auth directive — dufs has no auth and
  # trusts the local Docker network. This eliminates the Digest nonce round-
  # trip that caused seek stalls in Infuse and other FFmpeg-based players.
  cat > "${ROOT_DIR}/webdav/config.yml" <<'EOF'
# Generated by scripts/bootstrap-remote.sh. No credentials stored here.
# Authentication is handled at the Caddy layer (see Caddyfile /webdav block).
serve-path: /media
bind: 0.0.0.0
port: 6065
path-prefix: /webdav
...
compute_webdav_password_hash() {
  # Compute bcrypt hash of WEBDAV_PASSWORD for Caddy's basic_auth directive.
  # Uses the caddy image already pulled by compose (no extra download).
  local hash
  hash=$(docker_cmd run --rm caddy:2.9-alpine \
    caddy hash-password --plaintext "${WEBDAV_PASSWORD}")
  printf '%s' "${hash}"
}
...
main() {
  ...
  # and the generated webdav config both pick them up consistently.
